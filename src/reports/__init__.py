"""Scheduled reports — periodic LLM-generated summaries.

A report is a named job with a cron schedule, a prompt, and a list of
output channels (``teams``, ``notion``, ``vault``). At each firing the
runner gathers recent vault content + sync stats, asks the configured
LLM to generate a structured summary against the prompt, and pushes
the result to every configured channel.
